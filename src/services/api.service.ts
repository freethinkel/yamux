...
const loginWithWebView = async (): Promise<string> => {
  unlisten?.();
  return new Promise(async (resolve) => {
    await invoke('create_auth_window', {
      uri: `https://oauth.yandex.ru/authorize?response_type=code&client_id=23cabbbdc6cd418abb4b39c32c41195d`,
      label: 'oauth_window',
    });
    ...
    listen(
      ...
      ({ payload }: { payload: { url: string } }) => {
        if (payload.url.indexOf('https://music.yandex.ru/?code=') === 0) {
          authWindow.close();
          const searchParams = new URL(payload.url).searchParams;
...
export class ApiService {
  static async auth() {
    const code = await loginWithWebView();
    ...
    return httpClient.post('https://oauth.yandex.com/token', sp.toString(), {
      headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
    });
  }

  static getAccountStatus() {
    return httpRust.get(`${API_URL}/account/status`);
  }
